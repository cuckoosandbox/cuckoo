...
class PipeHandler(Thread):
    ...
    def run(self, configfile="analysis.conf"):
        """Run handler.
        ...
        if data:
            ...
            # having some stability issues with regards to webbrowsers.
                ...
                is_url = Config(cfg=configfile).category != "file"
...
class PipeServer(Thread):
    ...
    def __init__(self, pipe_name=PIPE, configfile="analysis.conf"):
        """
    ...
    def run(self):
        ...
        while self.do_run:
            ...
            if KERNEL32.ConnectNamedPipe(h_pipe, None) or KERNEL32.GetLastError() == ERROR_PIPE_CONNECTED:
                handler = PipeHandler(h_pipe, self.configfile)
                handler.daemon = True
                handler.start()
            else:
...
class Analyzer:
    ...
    def prepare(self, configfile="analysis.conf"):
        """Prepare env for analysis.
        @param configfile: The configfile to get all settings from
        """
        global DEFAULT_DLL
        ...
        self.config = Config(cfg=configfile)


        if not getattr(self.config, "no_time_change", False):
        ...
        for x in xrange(self.PIPE_SERVER_COUNT):
            self.pipes[x] = PipeServer(configfile)
        ...
        if self.config.category == "file":
            try:
                self.target = os.path.join(self.config.file_path,
    ...
    def run(self, configfile="analysis.conf"):
        ...
        pack = package_class(self.get_options(), configfile)

        # Initialize Auxiliary modules
        Auxiliary(configfile)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run the analysis on a sample')
    parser.add_argument('--configfile', help='The configfile to use', default="analysis.conf")
    ...
    try:
        ...
        success = analyzer.run(args.configfile)
