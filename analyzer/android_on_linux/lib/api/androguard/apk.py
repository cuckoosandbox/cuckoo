...
class APK:
    ...
    def __init__(self, filename, raw=False, mode="r", magic_file=None, zipmodule=ZIPMODULE):
        ...
        try:
            ...
            raise IOError("File is not a zip file")
    ...
    def get_files_types(self):
        ...
        try:
            ...
            for i in self.get_files():
                ...
                a["name"]=i
                a["type"]=self.files[i]
                a["md5"]=hashlib.md5(buffer).hexdigest()
                a["size"]=sys.getsizeof(buffer)
                self.files_md5.append(a)
            return self.files
    ...
    def get_files_with_md5(self):
        if self.files_md5 == []:
            self.get_files_types()

        return self.files_md5
    ...
    def get__extended_receivers(self) :
        """
        Return the android:name attribute of all receivers

        :rtype: a list of string
        """
        manifest =self.get_AndroidManifest().getElementsByTagName("manifest")[0]
        receivers = []
    ...
    def get_details_permissions_new(self) :
        """
            Return permissions with details
