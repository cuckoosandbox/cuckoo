...
class Analyzer(object):
    ...
    def prepare(self):
        ...
        if self.config.options.get("cert", None):
            try:
                args = [
                    "settings", "put", "global", "http_proxy",
                    self.config.options["proxy"]
                ]
                p = subprocess.Popen(
                ...
                log.error("Failed to configure http proxy: %s", e)

            shutil.copy(
                self.config.options["cert"], "/data/local/tmp/cert.crt"
            )
