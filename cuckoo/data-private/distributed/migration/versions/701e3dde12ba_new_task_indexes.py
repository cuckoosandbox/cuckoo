...
def upgrade():
    op.create_index('ix_completed_not_null', 'task', ['completed'],
                    unique=False,
                    postgresql_where=sa.text(u'completed IS NOT NULL'))
    op.create_index('ix_status_ltfinished_submitted', 'task',
                    ['submitted', 'status'], unique=False,
                    postgresql_where=sa.text(u"status < 'finished'"))
