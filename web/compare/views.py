...
class Left(LoginRequiredMixin, View):

    def get(self, request, left_id):
...
class Hash(LoginRequiredMixin, View):

    def get(self, request, left_id, right_hash):
        left = results_db.analysis.find_one({"info.id": int(left_id)},
                                            {"target": 1, "info": 1})
        if not left:
            return render_to_response("error.html",
...
class Both(LoginRequiredMixin, View):

    def get(self, request, left_id, right_id):
        left = results_db.analysis.find_one({"info.id": int(left_id)},
                                            {"target": 1, "info": 1})
